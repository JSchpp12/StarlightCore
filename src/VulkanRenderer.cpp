...
void star::core::VulkanRenderer::cleanup(){
    ...
    this->device.destroyBuffer(this->indexBuffer); 
...
void star::core::VulkanRenderer::createGraphicsPipeline() {
    common::Handle objectHandle = this->Renderer::objectList->at(0); 
    ...
    auto vertShaderCode = this->shaderManager->Get(currObject->getVertShader())->GetSpirV();
    auto fragShaderCode = this->shaderManager->Get(currObject->getFragShader())->GetSpirV();
    ...
    */
    ...
    //TODO: ONLY CREATING ONE PIPELINE RIGHT NOW
    this->graphicsPipeline = result.value.at(0);

    //destroy the shader modules that were created 
    this->device.destroyShaderModule(vertShaderModule); 
    this->device.destroyShaderModule(fragShaderModule); 
}
...
void star::core::VulkanRenderer::createVertexBuffer() {
    common::Object* currObject = this->objectManager->Get(this->objectList->at(0)); 
    ...
    auto verticies = currObject->getVerticies();
...
void star::core::VulkanRenderer::createIndexBuffer() {
    ...
    bufferSize = sizeof(indicies->at(0)) * indicies->size();
...
void star::core::VulkanRenderer::createCommandBuffers() {
    ...
    for (size_t i = 0; i < graphicsCommandBuffers.size(); i++) {
        ...
        */
        ...
        this->graphicsCommandBuffers[i].bindPipeline(vk::PipelineBindPoint::eGraphics, graphicsPipeline); 
        ...
        this->graphicsCommandBuffers[i].bindVertexBuffers(0, vertexBuffer, offsets);

        this->graphicsCommandBuffers[i].bindIndexBuffer(indexBuffer, 0, vk::IndexType::eUint32); 
        ...
        */
        ...
        this->graphicsCommandBuffers[i].bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayout, 0, 1, &this->descriptorSets[i], 0, nullptr); 
